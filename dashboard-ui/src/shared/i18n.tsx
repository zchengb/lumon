...
const zhCN = {
  ...
  "agent.langfuseDescription": "为每条已接受的飞书消息记录生命周期 Trace。凭据只保存在本机的 Agent 配置中。",
  ...
  "agent.captureContent": "捕获脱敏后的内容",
  "agent.captureContentHelp": "关闭时只发送 Trace 元数据；开启后会在发送前脱敏。",
...
const messages: Record<Locale, MessageSet> = {
  ...
  "zh-TW": {
    ...
    "agent.langfuseDescription": "為每條已接受的飛書訊息記錄生命週期 Trace。憑據只儲存在本機的 Agent 設定中。",
    ...
    "agent.captureContent": "擷取已遮罩內容",
    "agent.captureContentHelp": "關閉時只傳送 Trace 中繼資料；開啟後會在送出前遮罩。",
  ...
  en: {
    ...
    "agent.langfuseDescription": "Record a lifecycle trace for each accepted Feishu message. Credentials stay in the local Agent configuration.",
    ...
    "agent.captureContent": "Capture redacted content",
    "agent.captureContentHelp": "When off, only trace metadata is sent; when on, content is redacted before it is sent.",
