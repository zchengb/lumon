...
class AgentPlatformTests(unittest.TestCase):
    def test_definitions_registered(self) -> None:
        ...
        self.assertIn("irving", ids)
        dylan = get_definition("dylan")
        mark = get_definition("mark")
        milchick = get_definition("milchick")
        irving = get_definition("irving")
        assert dylan is not None and mark is not None and milchick is not None and irving is not None
        ...
        self.assertTrue(mark.capabilities.direct_workspace_write)
        self.assertEqual(dylan.capabilities.filesystem_mode, "workspace_write")
        self.assertEqual(mark.capabilities.filesystem_mode, "workspace_write")
        self.assertTrue(irving.capabilities.direct_workspace_write)
        self.assertEqual(irving.capabilities.filesystem_mode, "workspace_write")
