...
def _release(
    ...
    return ReleaseInfo("v1.0.7", version, assets), payloads
...
def test_update_installs_verified_wheel() -> None:
    ...
    result = UpdateService(source, installer, current_version="1.0.2").update(
        UpdateRequest("zchengb/lumon", python_version="3.12")
    )

    assert result.status is UpdateStatus.UPDATED
    assert result.current_version == "1.0.2"
    assert result.latest_version == "1.0.7"
    assert source.downloaded == ["lumon-1.0.7-py3-none-any.whl", "SHA256SUMS"]
...
def test_update_is_noop_when_current_version_is_latest() -> None:
    version = ReleaseVersion(1, 0, 17)
    release = ReleaseInfo("v1.0.17", version, ())
    ...
    assert result.current_version == "1.0.17"
    assert result.latest_version == "1.0.17"
    assert installer.installed_bytes is None
...
def test_update_rejects_a_checksum_mismatch() -> None:
    ...
    with pytest.raises(UpdateError, match="Checksum verification failed"):
        UpdateService(source, installer, current_version="1.0.2").update(
            UpdateRequest("zchengb/lumon")
        )
...
def test_python_venv_installer_uses_pip_without_uv(
    ...
    wheel = tmp_path / "lumon-1.0.7-py3-none-any.whl"
