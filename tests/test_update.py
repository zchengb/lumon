...
def _release(
    ...
    return ReleaseInfo("v1.0.5", version, assets), payloads
...
def test_update_installs_verified_wheel() -> None:
    ...
    assert result.latest_version == "1.0.5"
    assert source.downloaded == ["lumon-1.0.5-py3-none-any.whl", "SHA256SUMS"]
...
def test_update_is_noop_when_current_version_is_latest() -> None:
    version = ReleaseVersion(1, 0, 5)
    release = ReleaseInfo("v1.0.5", version, ())
    ...
    assert result.current_version == "1.0.5"
    assert result.latest_version == "1.0.5"
    assert installer.installed_bytes is None
...
def test_python_venv_installer_uses_pip_without_uv(
    ...
    wheel = tmp_path / "lumon-1.0.5-py3-none-any.whl"
