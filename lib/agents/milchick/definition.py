...
MILCHICK_DEFINITION = AgentDefinition(
    ...
    capabilities=AgentCapabilities(
        ...
        read_scopes=("agent_jobs", "schedules", "workflow_status", "jira", "test_case"),
        filesystem_mode="workspace_read",
        network_profile="deny",
        secret_profile="isolated",
        direct_workspace_write=False,
        allowed_workflows=("agent.job.create", "agent.job.show", "agent.health", "test_case.generate", *JIRA_ACTIONS),
        allowed_mutations=(
            "agent.job.create",
            "agent.job.cancel",
            "agent.job.retry",
