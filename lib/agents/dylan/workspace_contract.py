...
def _managed_block(project_slug: str) -> str:
    ...
    return (
        ...
        f"--finding <id> --json  (host/admin only; conversational path prefers the connected tool registry)\n\n"
        f"## Jira\n"
        f"- Prefer authorized `twg jira workitem get/query` for read-only Jira evidence; host reads remain the fallback\n"
        f"- Create/update work items through the connected tool registry when the latest request calls for that write\n\n"
        f"## Security Boundary\n"
        ...
        f"- Host mutations go through the host-side Capability Broker via connected tools\n"
        ...
        f"## Conversation Output\n"
        f"- Use ordinary Feishu messages for findings and final answers\n"
        f"- Use connected tools for mutations; do not print provider transport metadata\n\n"
