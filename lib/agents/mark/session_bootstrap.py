...
def build_bootstrap_prompt(
    *,
    ...
    return (
        ...
        "- Plan output: keep the first response, intermediate evidence, blockers, clarifications, and final plan in ordinary Feishu text. Never turn a plan-shaped response into a PDF or attachment unless the user explicitly asks for a file. Treat workspace output/pdf/*.pdf as ephemeral: for every explicit PDF or re-generation request, generate and attach the current document instead of skipping because an older PDF exists; the host removes the local PDF after transfer.\n"
