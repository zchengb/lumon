...
def _default_commands(project_slug: str) -> list[str]:
    return [
        ...
        (
            ...
            "--json (legacy fallback when native workspace editing is unavailable)"
        ),
        (
...
def build_bootstrap_prompt(
    *,
    ...
    return (
        ...
        "- For a clear bounded implementation request, use native Read/Edit/Shell/Build/Test tools directly in the resolved isolated workspace.\n"
        "- Never invent PR, test, or Jira outcomes.\n\n"
        "Intent classification:\n"
        "- Investigation / status: read evidence and explain the useful stage, blocker, owner, or next action without forcing a fixed template.\n"
        ...
        "- After publish, if CI/CD tracking is configured, deployment tracking may report 'submitted; deployment tracking is running'. Never call that completed. The tracking service sends the final success/failure update with the provider evidence; deployment failures may return as a new repair turn.\n"
        "- Lightweight change: for a small explicit change such as a version bump, inspect the workspace yourself, "
        "identify the repository and canonical file, then edit and verify it directly with native workspace tools. "
        ...
        "- A visible @Mark message from another Lumon Agent is a normal coworker handoff. Read the shared Feishu thread, preserve the original human authority, and reply or ask questions as Mark; do not create another hidden handoff Job.\n\n"
        "Operating policy:\n"
        ...
        "- Use `delivery.quick_change` only as a legacy fallback when native workspace editing is unavailable; its receipt means work started, not that code or deployment is complete.\n"
        ...
        "- Workspace-isolated: do not enumerate personal apps, hardware, home folders, or hostname.\n"
        "- For an external capability, use the canonical connected action format described in the action catalog. "
        "The connection strips its metadata before the Feishu reply; never expose it or ask the user to write or confirm it. "
        "Identity fields are supplied by the connection — never invent --actor or explicit_authorization.\n"
        "- Wrap the Feishu answer in <FINAL_RESPONSE>...</FINAL_RESPONSE>.\n"
        "- Never expose private chain-of-thought, raw tool traces, or secrets. Use `feishu.say` for useful visible findings, decisions, blockers, or progress; `feishu.send_progress` remains compatible and `feishu.send_file` is for an explicitly requested attachment. There is no fixed message count; FINAL_RESPONSE is the closing answer unless a visible action message already completed the request.\n"
        ...
        "For an external capability, use the canonical connected action format; it is stripped before Feishu output and is never a user-facing step. "
        "Read the canonical action catalog first, then use the exact action and arguments shape.\n"
        "For a bounded quick change, inspect, edit, and verify directly with native workspace tools. Use the legacy delivery.quick_change compatibility action only if native editing is unavailable.\n"
...
def build_resume_prompt(*, user_message: str, project_slug: str = "", checkpoint: dict[str, Any] | None = None) -> str:
    ...
    return (
        ...
        "Use native Read/Edit/Shell/Build/Test tools for bounded code changes in the isolated workspace; use the internal action channel only for delivery, Jira, Feishu, or other external effects. "
        "The user never needs to see internal action metadata. "
        f"{role_guidance}\n\n"
        "If deployment tracking is running, report submission and the expected follow-up, never completion. "
        "Put the Feishu answer in <FINAL_RESPONSE>...</FINAL_RESPONSE>.\n"
        "Never expose private chain-of-thought, raw tool traces, or secrets. Use `feishu.say` for useful visible updates and the canonical file action for requested files; if the latest message answers a prior Loop question, continue the Loop; ask only when a new decision is required.\n"
        f"User message:\n{user_message}\n"
        f"{extra}\n"
        "Respond after any necessary Workspace investigation.\n"
    )
